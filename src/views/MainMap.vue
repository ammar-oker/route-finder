        ...
        .then(passengers => {
          ...
          this.getRouteWithWayPoints(this.startingPoint, this.destinationPoint, this.passengers)
            .then((data) => {
              ...
              this.$store.dispatch('setEnrichedPassengers', data.enrichedPassengers);
              this.$store.dispatch('setTotalDistance', data.totalDistance);
              this.$store.dispatch('setTotalDuration', data.totalDuration);
              this.$store.dispatch('setRouteReady', true);
            })
            .catch(error => {
              this.reset();
              this.error = error;
          ...
          this.geocoder.geocode({ location: this.startingPoint }, (results, status) => {
            ...
            this.$store.dispatch('setOriginPoint', results[0].formatted_address);
          });
          this.geocoder.geocode({ location: this.destinationPoint }, (results, status) => {
            if (status !== 'OK' || !results[0]) {
              throw new Error(status);
            }
            this.$store.dispatch('setDestinationPoint', results[0].formatted_address);
          });
        });
    },
